...
def play_video():
    video = cv2.VideoCapture("./data/out_Data.mp4")
...
keypoints_button = tk.Button(window, text="显示图片检测结果", command=show_keypoints)
keypoints_button.pack(side=tk.LEFT)
keypoints_button = tk.Button(window, text="清除", command=clearAll)
keypoints_button.pack(side=tk.LEFT)
...
open_button.pack(side=tk.RIGHT)
open_button = tk.Button(window, text="显示视频检测结果", command=play_video)
open_button.pack(side=tk.RIGHT)
window.mainloop()
